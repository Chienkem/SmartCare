...
import apiServiceRefrigerator from '../api/apiServiceRefrigerator';
const { Option } = Select;
...
function Refrigerator(){
   ...
   useEffect(() => {
    const postData = async () => {
      try {
        const res = await apiServiceRefrigerator.post();
...
const handleOk = () => {
  ...
  if (isEdit) {
    if(checkNullValue(editData)){
      ...
      const putData = async () => {
        try {
          const res = await apiServiceRefrigerator.put(editData.id, editData);
          resetFormData()
          setDataSource((pre) => {
            return pre.map((item) => {
    ...
    //   const postData = async () => {
    //     try {
    //       const res = await apiServiceRefrigerator.post("insert",addData);
    //       resetFormData();
...
const handleChangeSearch =(e)=>{
  const postData = async () => {
    try {
     const res = await apiServiceRefrigerator.post("search",{
...
const handleChangeStatus = (e)=>{
  const postData = async () => {
    try {
      const res = await apiServiceRefrigerator.post("status",{status:e});
      resetFormData();
      setDataSource(res.rows)
    } catch (err) {
      console.log(err);
  ...
  <ContentContainer >
    <HeaderContent style={{float: 'right'}}>
      ...
      <div style={{display: "flex"}}>
          ...
          <FilterAddress 
            ApiComponent={apiServiceRefrigerator}
    ...
    <Table
      ...
      pagination={{
        ...
        onChange: (page, pageSize) => {
          ...
          const postData = async () => {
            try {
              const res = await apiServiceRefrigerator.post(page);
              setDataSource(res.rows)
              setTotal(res.count)
            } catch (err) {

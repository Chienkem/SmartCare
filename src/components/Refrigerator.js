...
function Refrigerator(){
  ...
  const[dataRefrigerator, setDataRefrigerator] = useState([]);
  useEffect(() => {
    axios.get('https://61e51bf0595afe00176e5310.mockapi.io/api/v1/waterfilter')
    .then(res => {
      setDataRefrigerator(res.data);
    })
    .catch(err => {
      console.log(err);
    })
  ...
  const columns = [
    ...
    {
      title: 'Người dùng',
      dataIndex: 'nameRefrigerator',
      key: 'nameRefrigerator',
    },
    {
      title: 'Mã thiết bị',
  ...
  const [serviceRequestData, setRefrigeratorRequestData] = useState({});
  const handerChangeRefrigeratorRequestData = (e) => {
    const name = e.target.name;
    const value = e.target.value;
    setRefrigeratorRequestData({...serviceRequestData, [name]: value});
  }
  ...
  const handleOk = () => {
    ...
    .then(res => {
      setDataRefrigerator(pre => [...pre, res.data]);
    })
    .catch(err => {
      console.log(err)
    })
  ...
  const handerDelete = (record) => {
    Modal.confirm({
      ...
      onOk() {
        ...
        .then(res => { 
          setDataRefrigerator(pre => pre.filter(item => item.id !== record.id)); 
        })
      },
      onCancel() {
        // console.log('Cancel');
        ...
        <Row justify="space-between" style={{paddingBottom: "20px"}}>
          <Col span={11}>
            ...
            <Input
              ...
              name="nameRefrigerator"
              onChange={e => handerChangeRefrigeratorRequestData(e)}
            />
          </Col>
          <Col span={11}>
          ...
          <Col span={11}>
            ...
            <Select
              ...
              style={{ width:"100%" }}
              name="status"
              onChange={e => setRefrigeratorRequestData({...serviceRequestData, status: e})}
            >
          ...
          <Col span={11}>
            ...
            <Select
              ...
              style={{ width:"100%" }}
              name="deviceType"
              onChange={e => setRefrigeratorRequestData({...serviceRequestData, deviceType: e})}
            >

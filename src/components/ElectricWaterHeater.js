...
function ElectricWaterHeader(){

  // lấy dữ liệu từ server
  const[dataElectricWaterHeader, setDataElectricWaterHeader] = useState([]);
  useEffect(() => {
    axios.get('https://61e51bf0595afe00176e5310.mockapi.io/api/v1/waterfilter')
    .then(res => {
      setDataElectricWaterHeader(res.data);
    })
    .catch(err => {
      console.log(err);
    })
  ...
  const columns = [
    ...
    {
      title: 'Người dùng',
      dataIndex: 'nameElectricWaterHeader',
      key: 'nameElectricWaterHeader',
    },
    {
      title: 'Mã thiết bị',
  ...
  const [serviceRequestData, setElectricWaterHeaderRequestData] = useState({});
  const handerChangeElectricWaterHeaderRequestData = (e) => {
    const name = e.target.name;
    const value = e.target.value;
    setElectricWaterHeaderRequestData({...serviceRequestData, [name]: value});
  }
  ...
  const handleOk = () => {
    ...
    .then(res => {
      setDataElectricWaterHeader(pre => [...pre, res.data]);
    })
    .catch(err => {
      console.log(err)
    })
  ...
  const handleCancel = () => {
    setIsModalVisible(false);
    setElectricWaterHeaderRequestData({});
  };
  ...
  const handerDelete = (record) => {
    Modal.confirm({
      ...
      onOk() {
        ...
        .then(res => { 
          setDataElectricWaterHeader(pre => pre.filter(item => item.id !== record.id)); 
        })
      },
      onCancel() {
        // console.log('Cancel');
  ...
  return(
    <ContentContainer >
      ...
      <Table
        columns={columns}
        dataSource={dataElectricWaterHeader}
        ...
        <Row justify="space-between" style={{paddingBottom: "20px"}}>
          <Col span={11}>
            ...
            <Input
              ...
              name="nameElectricWaterHeader"
              onChange={e => handerChangeElectricWaterHeaderRequestData(e)}
            />
          </Col>
          <Col span={11}>
          ...
          <Col span={11}>
            ...
            <Select
              ...
              style={{ width:"100%" }}
              name="status"
              onChange={e => setElectricWaterHeaderRequestData({...serviceRequestData, status: e})}
            >
          ...
          <Col span={11}>
            ...
            <Select
              ...
              style={{ width:"100%" }}
              name="deviceType"
              onChange={e => setElectricWaterHeaderRequestData({...serviceRequestData, deviceType: e})}
            >
...
export default ElectricWaterHeader;